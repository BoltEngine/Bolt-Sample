...
public static class BoltUpdater
{
	...
	[MenuItem("Bolt/Utils/Save Prefabs", priority = 127)]
	...
	[MenuItem("Bolt/Utils/Load Prefabs", priority = 128)]
	...
	[MenuItem("Bolt/Utils/Save Prefabs", priority = 127, validate = true)]
	private static bool ValidateSerializePrefabs()
	{
		return VerifyVersion(true);
	}

	[MenuItem("Bolt/Utils/Load Prefabs", priority = 128, validate = true)]
		...
		if (result != null)
		{
			...
			foreach (var prefab in prefabList)
			{
				foreach (var updateItem in result)
				{
					...
					{
						BoltLog.Info("Updating {0}", prefab);
	...
	private static void UpdateBoltEntity(GameObject target, UpdateItem item)
	{
		...
		{
			var entity = target.AddComponent<BoltEntity>().ModifySettings();

			entity.prefabId = entityComponent.prefabId;
			entity.sceneId = entityComponent.sceneId;
			entity.serializerId = entityComponent.serializerId;
			entity.updateRate = entityComponent.updateRate;
			entity.autoFreezeProxyFrames = entityComponent.autoFreezeProxyFrames;
			entity.clientPredicted = entityComponent.clientPredicted;
			entity.allowInstantiateOnClient = entityComponent.allowInstantiateOnClient;
			entity.persistThroughSceneLoads = entityComponent.persistThroughSceneLoads;
			entity.sceneObjectDestroyOnDetach = entityComponent.sceneObjectDestroyOnDetach;
			entity.sceneObjectAutoAttach = entityComponent.sceneObjectAutoAttach;
			entity.alwaysProxy = entityComponent.alwaysProxy;
		}

		if (item.hasEntityHitboxBody)
		{
			target.AddComponent<BoltHitboxBody>();
		}

		foreach (var hitBoxComponent in item.entityHitboxes)
		{
			var hitBox = target.AddComponent<BoltHitbox>();

			hitBox.hitboxShape = hitBoxComponent.hitboxShape;
			hitBox.hitboxType = hitBoxComponent.hitboxType;
			hitBox.hitboxSphereRadius = hitBoxComponent.hitboxSphereRadius;
			hitBox.hitboxCenter = hitBoxComponent.hitboxCenter;
			hitBox.hitboxBoxSize = hitBoxComponent.hitboxBoxSize;
		}
